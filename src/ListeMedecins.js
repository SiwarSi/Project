    ...
    <View style={[styles.container, {
      ...
      <View style={[{ flex: 7, backgroundColor: "#FFFFFF", marginLeft: 30, marginRight: 30, borderRadius: 20, borderColor: '#FAE7E6' }, { flexDirection: "column" }]}>
        <View style={[styles.bord, { flexDirection: "row" },]}>
          <Text style={{ marginLeft: width * 0.07, marginTop: height * 0.05, fontFamily: 'poppins', color: "#E3807B", fontWeight: 'bold', fontSize: 20 }}>Ajouter Un Medecins</Text>
        ...
        <ScrollView>
          {Medecins.map((item) => {
            return (
              <TouchableOpacity onPress={() => navigation.navigate('ProfileMedecin')}>
                <View key={item.Key} style={[styles.item, { flexDirection: "row" }]}>

                  <Text style={styles.items}>{item.name}</Text>
                  <TouchableOpacity>
                    <Image style={{ marginLeft: width * 0.2, marginTop: height * 0.02 }}
